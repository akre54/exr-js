...
import { EXRReader, decodeRgba } from '../src/index.js';
import { readFileSync } from 'fs';

test('read tiled single-layer EXR', () => {
  const reader = new EXRReader(readFileSync('test/outputs/test-builder-single.exr'));
...
test('read tiled EXR with simple API', () => {
  const result = decodeRgba(readFileSync('test/outputs/test-builder-single.exr'));
...
test.skip('read mipmap EXR', () => {
  const reader = new EXRReader(readFileSync('test/outputs/test-mipmap-down.exr'));
